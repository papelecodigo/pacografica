...
function addCss(){if(document.querySelector('link[data-check-wa-v9]'))return;const l=document.createElement('link');l.rel='stylesheet';l.href='./checklist-whatsapp-v9.css?build=20260901-1148';l.dataset.checkWaV9='1';document.head.appendChild(l)}
...
function setQr(dataUrl){const img=$('waQrImage'),loading=$('waQrLoading');if(!img||!loading)return;if(dataUrl){img.src=dataUrl;img.classList.remove('wa-hidden');loading.classList.add('wa-hidden')}else{img.removeAttribute('src');img.classList.add('wa-hidden');loading.classList.remove('wa-hidden')}}
async function refreshWhatsapp(){try{const s=await localFetch('/status');if(s.state==='ready'){setStatus('ready','WhatsApp conectado',s.account||'Pronto para importar clientes.');showOnly('waChatsArea');await loadChats();stopPoll()}else if(s.state==='qr'){setStatus('qr','Escaneie o QR Code','Aguardando conexão do WhatsApp.');showOnly('waQrArea');const q=await localFetch('/qr');setQr(q.dataUrl||null);startPoll()}else{setStatus('',s.state==='starting'?'Iniciando WhatsApp...':'Aguardando WhatsApp',s.message||'');showOnly('waQrArea');setQr(null);startPoll()}}catch{setStatus('','Conector local não encontrado','Inicie o conector neste computador.');showOnly('waOffline');setQr(null);stopPoll()}}
function startPoll(){if(pollTimer)return;pollTimer=setInterval(refreshWhatsapp,1800)}function stopPoll(){clearInterval(pollTimer);pollTimer=null}
...
function openWhatsapp(){mountDialog();$('waLocalDialog').showModal();setQr(null);refreshWhatsapp()}
